### once range sites have been established, combine with stream sites, i.e. run ga just for stream where the fitness function has ### a static aggregate range power curve
...
s_power = ds_stream_pow.power.values




not_nan = pandas.read_csv('data/processed/1e8.csv')
# not_nan = not_nan[not_nan['flag']=='r']
not_nan = not_nan[not_nan['flag']=='s']
df_idx = list(not_nan.index)


range_sol = pandas.read_csv('data/processed/ga-21-sol-modified.csv')
range_aggregate = 0
for i in range(20):
    curve = r_power[:,range_sol.iloc[i]['lat_index'],range_sol.iloc[i]['lon_index']]
    range_aggregate += curve
